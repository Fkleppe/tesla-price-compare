...
interface ModelPageInteractiveProps {
  model: {
    id: string;
    name: string;
    shortName: string;
...
export default function ModelPageInteractive({ model, initialProducts }: ModelPageInteractiveProps) {
  ...
  const availableStores = useMemo(() => {
    ...
    return [...new Set(modelProducts.map(p => p.source))].sort();
  ...
  const hasFiltersActive = selectedCategories.length > 0 || selectedStores.length > 0 || onSaleOnly || priceMin > 0 || priceMax < 5000;
